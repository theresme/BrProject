      ...
      <ul className="mt-3 space-y-2">
        {pesos.map((p) => (
          <li key={p.instituto} className="flex items-center gap-3 text-sm">
            <span className="w-40 truncate text-gray-300" title={p.instituto}>
              {p.instituto}
              {!p.registradaTse && (
                <span className="ml-1 text-gray-600" title="sem registro TSE localizado">*</span>
              )}
            </span>
            <div className="h-2 flex-1 overflow-hidden rounded bg-ink">
              <div className="h-full rounded bg-brasil"
                   style={{ width: `${p.rating * 100}%` }} />
            </div>
            <span className="num w-10 text-right text-gray-400">
              {p.rating.toFixed(2)}
            </span>
          </li>
        ))}
